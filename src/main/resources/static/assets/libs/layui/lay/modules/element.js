  ...
  ,call = {
    ...
    */
    ...
    ,clickThis: function(){
      ...
      //如果是垂直菜单
          // TODO EasyWeb 侧边栏展开、折叠增加过渡效果
          if (parents.hasClass(NAV_TREE)) {
            child.removeClass(NAV_ANIM);

            // 如果有子菜单，则展开
            if (child[0]) {
              if (parent.hasClass('admin-nav-hover')) {
                return;
              }
              /*var layAccordion = false;
              if (parent.parent().hasClass('layui-nav-tree')) {
                layAccordion = parent.parent().attr('lay-accordion') == 'true';
              } else {
                layAccordion = parent.parentsUntil('.layui-nav-tree').parent().attr('lay-accordion') == 'true';
              }*/
              var _this = this;
              if (!parent.hasClass('layui-nav-itemed')) {
                if (parents.attr('lay-shrink') === 'all') {
                  // parent.siblings().removeClass(NAV_ITEM + 'ed');
                  parent.parent().children('.layui-nav-itemed').not(parent).children('.layui-nav-child').slideUp('fast');
                  parent.parent().children('.layui-nav-itemed').not(parent).removeClass(NAV_ITEM + 'ed');
                }
                parent.children('.layui-nav-child').slideDown('fast', function () {
                  layui.event.call(_this, MOD_NAME, 'nav(' + filter + ')', othis);
                  parent.trigger('mouseenter');
                });
                parent['addClass'](NAV_ITEM + 'ed');
              } else {
                parent.children('.layui-nav-child').slideUp('fast', function () {
                  layui.event.call(_this, MOD_NAME, 'nav(' + filter + ')', othis);
                  parent.trigger('mouseenter');
                });
                parent['removeClass'](NAV_ITEM + 'ed');
              }
            } else {
              layui.event.call(this, MOD_NAME, 'nav(' + filter + ')', othis);
            }
    ...
    */
    ...
    ,collapse: function(){
      ...
      //是否手风琴
          // TODO EasyWeb 折叠、展开增加过渡效果
          if (typeof parents.attr('lay-accordion') === 'string') {
            var show = parents.children('.layui-colla-item').children('.' + SHOW).not(elemCont);
            if (isIE8()) {
              show.siblings('.layui-colla-title').children('.layui-colla-icon').html('&#xe602;');
            } else {
              show.siblings('.layui-colla-title').children('.layui-colla-icon').html('&#xe602;');
              show.siblings('.layui-colla-title').children('.layui-colla-icon').css({
                'transition': 'all .3s',
          ...
          if (isNone) {
            elemCont.slideDown('fast', function () {
              $(this).removeAttr("style");
              elemCont['addClass'](SHOW);
            });
          } else {
            elemCont.slideUp('fast', function () {
              $(this).removeAttr("style");
          ...
          if (isIE8()) {
            ...
            icon.html('&#xe602;');
            var deg = isNone ? '90deg' : '0deg';
            icon.css({'transition': 'all .3s', 'transform': 'rotate(' + deg + ')'});
          }
      /*if(typeof parents.attr('lay-accordion') === 'string'){
